...
covid_df.where('date == "2020-03-31"').where('iso_code not like "OWID%"').withColumn("percent", covid_df.total_cases/covid_df.population*100).select("iso_code", "location", "percent").orderBy(col("percent").desc()).limit(15).write.mode("overwrite").csv('15perc.csv')
