...
df = df.where('iso_code not like "OWID%"').select("date", "location", "new_cases").withColumn("date", to_date(col("date"), "yyyy-MM-dd")).withColumn("new_cases", df.new_cases.cast('int')).where("date between '2021-03-29' and '2021-03-31'")
...
df_final.write.mode("overwrite").option("header", True).csv('2.csv')